...
# Or manually:
cd backend && source .venv/bin/activate && uvicorn gls.main:app --reload
...
make test-frontend  # vitest (when configured)
make lint-backend   # ruff + mypy
make lint-frontend  # eslint + svelte-check
```
...
# 2. Process all new talks (recommended - does everything)
make process-new PROVIDER=openai

# Or step by step:
make align-all     # Transcribe audio with OpenAI Whisper API → alignment.json
make clean-all PROVIDER=openai  # Clean PDFs with GPT-4 → text.txt
...
**Pipeline order**:
1. `align-all` - OpenAI Whisper API transcribes audio → `alignment.json` (transcript with word timestamps)
2. `clean-all` - GPT-4 cleans PDF text guided by transcript → `text.txt` + `cleaning_notes.json`

**Environment variables** (set in `.env` file):
- `OPENAI_API_KEY` - Required for both transcription and text cleaning
- `ANTHROPIC_API_KEY` - Alternative for text cleaning (default provider if set)
- `PROVIDER=openai` - Use OpenAI for text cleaning (recommended)
...
├── {lang}/
│   ├── *.mp3           # Audio file
│   ├── *.txt           # Official text (cleaned from PDF)
│   └── alignment.json  # WhisperX transcript with timestamps
```

### Transcript vs Official Text (Design Note)

We maintain **two text sources** for each language version:

1. **Official text** (`*.txt`) - High-quality translation from church publications
2. **Transcript** (`alignment.json` segments) - WhisperX transcription of actual audio

These may differ because speakers ad-lib, skip sections, or speak differently than
the written text. Both are valuable for language learners:

- **Official text**: Polished translation, good for reading comprehension
- **Transcript**: What was actually said, essential for listening practice

**Current MVP**: Shows transcript during playback (has accurate timestamps).

**Future features** (post-MVP):
- Toggle between transcript and official text views
- Side-by-side comparison mode
- AI-assisted mapping between transcript and official text
- Cross-language segment mapping (align English segment to Chinese equivalent)
- Feed both to AI for contextual translation help
...
### Scripts
- `scripts/transcribe_openai.py` - OpenAI Whisper API transcription (recommended)
- `scripts/generate_alignment.py` - Local WhisperX transcription (has PyTorch issues)
- `scripts/clean_text.py` - LLM-based PDF cleaning (uses transcript as guide)
- `scripts/preprocess_pdf.py` - Basic regex PDF cleaning (legacy)
- `Makefile` - Project automation (dev, test, lint, align, clean, etc.)
